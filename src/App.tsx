...
const PHOTO_INTEGRATION_RULES = {
  ...
  '6': "PHOTO INTEGRATION [STYLE 6 - DUAL STATE]: Use the SAME FACELESS CHARACTER in TWO STATES side-by-side. BEFORE SIDE: Slumping posture, closed body, dull/dark lighting, noise. AFTER SIDE: Upright posture, open body, bright/clean lighting. The character design must be identical, only posture/lighting changes. CRITICAL RULES: The face MUST be completely hidden/obscured in BOTH states (using shadows, objects, or turned away). Eyes/nose/mouth must NOT be visible! If original photo has GLASSES, keep them on both sides."
};
...
export default function App() {
  ...
  const generatePrompt = () => {
    ...
    const facelessRule = isIllustrationStyle ? 
      ...
      "FACELESS RULE: The entire face MUST be COMPLETELY hidden/obscured in heavy pitch-black shadows. EYES, NOSE, and MOUTH MUST NOT BE VISIBLE under any circumstances. The shadow must completely swallow all facial features, or the face must be blocked by an object. DO NOT just add light shadows where the face is still recognizable. DO NOT use smooth featureless blank skin.";
